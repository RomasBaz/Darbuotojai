...
class All_data():
    ...
    def update_record(self):
        ...
        try:
            ...
            c = conn.cursor()
            with conn:
                c.execute(f'UPDATE darbuotoju_duomenys SET Vardas="{self.field_name.get()}" WHERE id={selected}')
                c.execute(f'UPDATE darbuotoju_duomenys SET Pavardė="{self.field_last_name.get()}" WHERE id={selected}')
                c.execute(f'UPDATE darbuotoju_duomenys SET Gimimo_data="{self.field_birth_date.get()}" WHERE id={selected}')
                c.execute(f'UPDATE darbuotoju_duomenys SET Lytis="{self.field_gender.get()}" WHERE id={selected}')
                c.execute(f'UPDATE darbuotoju_duomenys SET Adresas="{self.field_address.get()}" WHERE id={selected}')
                c.execute(f'UPDATE darbuotoju_duomenys SET Pašto_kodas="{self.field_zip_code.get()}" WHERE id={selected}')
                c.execute(f'UPDATE darbuotoju_duomenys SET Pareigos="{self.field_duties.get()}" WHERE id={selected}')
                c.execute(f'UPDATE darbuotoju_duomenys SET Atlyginimas="{self.field_salary.get()}" WHERE id={selected}')
